...
def find_streaming(
    ...
    **stac_kwargs
    ...
    if "base_catalog_href" not in stac_kwargs:
        if engine == "stac":
            ...
            if partition_type == "h3":
                ...
                default_catalog = f"s3://its-live-data/test-space/stac/geoparquet/h3r{resolution}"
            ...
            raise ValueError(f"Invalid engine: {engine}. Must be 'stac', 'duckdb', or 'rustac'.")
    ...
    # Remove None filters
    stac_params["filters"] = {k: v for k, v in stac_params["filters"].items() if v is not None}
